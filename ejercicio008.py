...
def main(digitos):
    ...
    mayor=0
    producto=1
    #----------definimios data donde buscaremos
    ...
    for idx in range(int(len(data))):
        ...
        digitos+=1
        temp=[]
        #--------creamos las convinaciones en la data
        for id in range(idx,digitos): 
            #------cargamos los digitos en una tabla temporal ||||| [id-flag] ----> convierte nuestra data de manera linial a cicular obetniendo todas las convinaciones              
            temp.append(data[id-flag])
        # print(temp)
        #------cargamos la suma de los digitos 
        suma=sumaLista(temp)
        #-----seleccionamos el valor mas alto de la suma de los digitos
        if mayor<suma :
            mayor=suma
            rpta=temp
        # rpta=[temp, suma]
    for t in range (int(len(rpta))):
        producto=producto* int(rpta[t])
    k="".join(rpta)
    print(f"en numero es {k}")
    print(f"la suma de sus cifras es : {mayor}")
    print(f"el producto de sus cifras es: {producto}")
#-------------------ejecutar la funcion main y en el parametro colocar la cantidad de cifras del numero              
