#ENUNCIADO:
#  Los cuatro dígitos adyacentes en el número de 1000 dígitos que tienen el mayor producto son 9 × 9 × 8 × 9 = 5832.



#  Encuentra los trece dígitos adyacentes en el número de 1000 dígitos que tienen el mayor producto. ¿Cuál es el valor de este producto?
#--------------calcular la suma de una lista
def productoLista(q):
    q=q
    producto=1
    for m in range(int(len(q))):
        producto=producto*int(q[m])
    return(producto)

# print (productoLista([8,6,1]))

def main(digitos):
    digitos=digitos-1
    flag=digitos
    # suma=0
    rpta=[]
    mayor=0
    # producto=1
    #----------definimios data donde buscaremos
    data=""
    data=list(data)
    #--------recorremos la data
    for idx in range(int(len(data))):
        # print(idx)
            
        digitos+=1
        temp=[]
        #--------creamos las convinaciones en la data
        for id in range(idx,digitos): 
            #------cargamos los digitos en una tabla temporal ||||| [id-flag] ----> convierte nuestra data de manera linial a cicular obetniendo todas las convinaciones              
            temp.append(data[id-flag])
            # print(data[id-flag])
        # print(temp)
        #------cargamos el producto de los digitos 

        prod=productoLista(temp)
        # print(prod)
        #-----seleccionamos el valor mas alto del producto de los digitos
        if mayor<prod :
            mayor=prod
            rpta=temp
    #-----------------devolvemos solucion
    k="".join(rpta)
    print(f"en numero es {k}")
    print(f"la suma de sus cifras es : {mayor}")
#-------------------ejecutar la funcion main y en el parametro colocar la cantidad de cifras del numero              

main(4)